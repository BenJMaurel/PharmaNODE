...
parser.add_argument("--generate_samples", action='store_true', help="Generate synthetic samples")
...
if __name__ == '__main__':  
    ...
    ckpt_path = os.path.join(args.save, "experiment_" + str(experimentID) + '_best.ckpt')    
    start = time.time()
    ...
    for itr in range(0,num_batches):
        with torch.no_grad():
    ...
    reconstructions = np.concatenate(all_reconstructions, axis = 1)
    ...
    reconstructions = torch.nan_to_num(torch.tensor(reconstructions), nan=0.0)
    ...
    print("Step 1: Running PCA on the latent means...")
    ...
    print("PCA embedding created.")
    
    # Step 2: Set up the Matplotlib figure for a static PNG
    print("Step 2: Generating static publication-ready plot...")
    
    # Create a figure with a standard aspect ratio
    fig, ax = plt.subplots(figsize=(10, 6), dpi=300) 
    
    # Calculate color limits based on real data to prevent outliers from ruining the scale
    # We use the 5th and 95th percentiles to keep the colors rich and vibrant
    real_labels = np.array(all_labels).squeeze()
    vmin = np.percentile(real_labels, 5)
    vmax = np.percentile(real_labels, 95)

    # Step 3: Plot the data layers
    if args.generate_samples:
        # 1. Plot the synthetic data first as a soft background layer
        num_samples = samples.shape[0]
        embedding_gen = pca.transform(samples)
        
        ax.scatter(
            embedding_gen[:, 0], embedding_gen[:, 1], 
            color='lightgrey', alpha=0.4, s=15, 
            label='Generated Samples', zorder=1
        )
        
        # 2. Plot the real data on top
        scatter = ax.scatter(
            embedding_real[:, 0], embedding_real[:, 1], 
            c=real_labels, cmap='RdBu_r', 
            vmin=vmin, vmax=vmax,
            alpha=0.85, s=30, edgecolor='white', linewidth=0.2, 
        ...
        scatter = ax.scatter(
            ...
            c=real_labels, cmap='RdBu_r', 
            vmin=vmin, vmax=vmax,
            alpha=0.85, s=30, edgecolor='white', linewidth=0.2, zorder=2
        )

    # Step 4: Formatting and aesthetics
    # Add a colorbar linked to the real data
    cbar = plt.colorbar(scatter, ax=ax)
    cbar.set_label('Label Value', fontsize=12)

    # Set titles and labels
    ax.set_title('PCA of Latent Space', fontsize=14, pad=15)
    ...
    output_filename = f"KEL_latent_space_pca_{experimentID}.png"
    plt.savefig(output_filename, format='png', bbox_inches='tight')
    print(f"Step 5: Plot successfully saved as '{output_filename}'!")
    
    # Optionally display it if you are running this interactively
    # plt.show()