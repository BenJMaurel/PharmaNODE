...
parser.add_argument('--exp', type=str, default='exp_film_run', help="Folder name for the FiLM dataset (inside ./results/)")
...
def calculate_auc(concentrations, times, static_treatments):
    ...
    for i in range(concentrations.shape[0]):
        ...
        aucs.append(np.trapezoid(valid_concs, valid_times))
...
if __name__ == '__main__':
    ...
    print(f"Loading testing dataset from ./results/exp_film_run/{args.exp}/virtual_cohort_film_test.csv...")
    # 1. Load Data
    data_dict_test, scaler_info = extract_gen_tac_film(
        file_path=[f"./results/exp_film_run/{args.exp}/virtual_cohort_film_train.csv",
                   f"./results/exp_film_run/{args.exp}/virtual_cohort_film_test.csv"]
    )
    ...
    with torch.no_grad():
        for batch in test_dataloader:
            ...
            t_aucs_v1 = (batch_dict["auc_red_v1"] * scaler_info[0]).cpu().numpy()
            t_aucs_v2 = (batch_dict["auc_red_v2"] * scaler_info[0]).cpu().numpy()
