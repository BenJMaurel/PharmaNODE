...
run_one_id <- function(patient_id, all_raw_data, obs_data) {
  ...
  # Build the estimation dataset safely using tryCatch
  est_obj <- tryCatch({
    mod_tac_updated %>%
    ...
    # If the ODE solver crashes, log the error and return NULL so the loop survives
    warning(sprintf("\n[!] Simulation failed for ID %s: %s\n", patient_id, e$message))
    return(NULL)
  })

  # If est_obj is NULL (because it failed), exit the function early
  ...
  auc_ipred <- MESS::auc(ipred_win_unique$time, ipred_win_unique$DV)
...
# Added the seed = TRUE option here to fix the furrr warning
analysis_output <- future_map(
  id_list, 
  ~run_one_id(.x, all_raw_data = raw_data, obs_data = obs_data_to_process), 
  .options = furrr_options(seed = TRUE),
  .progress = TRUE
)

# --- Process and Save Results ---
# Clean out any NULLs (patients that crashed) before plotting
