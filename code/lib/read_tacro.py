...
def extract_gen_tac(file_path=["virtual_cohort_train.csv", "virtual_cohort_test.csv"], plot=True, exp=None, sparse_times=[0., 1., 3.]):
...
def extract_gen_tac_film(file_path=["virtual_cohort_film_train.csv", "virtual_cohort_film_test.csv"], exp=None, sparse_times=[0., 1., 3.]):
    if exp:
...
class TacroFilmDataset(torch.utils.data.Dataset):
    ...
    def n_covariates(self):
        if not self.patient_ids:
            return 0
        return len(self.data_dict[self.patient_ids[0]]['v1'].get('static', []))
...
def collate_fn_tacro_film(batch, device):
    ...
    tp_obs_v1 = batch[0][2].clone().to(device)
    ...
    tp_obs_v2 = batch[0][nobv+2].clone().to(device)
...
def collate_fn_tacro(batch, args, device, data_type = "train"):
    ...
    if batch[0][-1].ndim == 0:
        ...
        static2 = (torch.stack([batch[i][2] for i in range(len(batch))]) - tp_obs)
    else:
