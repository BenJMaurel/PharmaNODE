...
def plot_auc(ax, data, reconstructions, times, times_pred, auc_be=None, auc_red=None, labels=None, plot_all = True, save_csv = True, patient_ids = None, args = None):
    ...
    if plot_all:
        for j in range(pages):
            ...
            try:
                plt.savefig(f"./results/{args.load}/all_predict_NODE_TAC_improved_{str(j)}.png", dpi=300, bbox_inches="tight")
            except:
                pass
            # plt.show()
    ...
    if save_csv:
        ...
        final_observations_df.to_csv(f"./results/{args.load}/observations.csv", index=False)
    ...
    predicted = np.array([np.trapezoid(predict.squeeze(-1), times_pred) for predict in predicted])
    # Scatter plot
