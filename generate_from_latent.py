...
if __name__ == '__main__':  
    ...
    if args.generate_samples == True:
        ...
        df = pd.DataFrame({
            'x': embedding[:, 0],
            'y': embedding[:, 1],
            # Removed the *2-1 to keep your true distribution
            'label': np.concatenate((np.array(all_labels).squeeze(0), np.zeros(num_samples))), 
            'patient_id': np.arange(num_samples + len(all_latent_z0_means)),
        })
    else: 
        embedding = embedding_real
        df = pd.DataFrame({
            'x': embedding[:, 0],
            'y': embedding[:, 1],
            # Removed the *2-1 to keep your true distribution
            'label': np.array(all_labels), 
            'patient_id': np.arange(len(all_latent_z0_means)),
        })
    color_max = np.percentile(df['label'], 95)
    color_min = df['label'].min()
    ...
    fig = go.Figure(data=[go.Scatter(
        ...
        marker=dict(color=df['label'], colorscale='viridis', showscale=True, colorbar_title='Label Value', size=8, opacity = 0.5, cmin=color_min,
            cmax=color_max)
    )])
